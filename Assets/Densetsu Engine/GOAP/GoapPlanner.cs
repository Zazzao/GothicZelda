        ...
        public ActionPlan Plan(GoapAgent agent, HashSet<AgentGoal> goals, AgentGoal mostRecentGoal = null) {
            ...
            foreach (var goal in orderedGoals) {
                ...
                if (FindPath(goalNode, agent.actions)) {
                    // If the goalnode has no leaves and no action to perform try different goal
        ...
        private bool FindPath(Node parent, HashSet<AgentAction> actions) {
            foreach (var action in actions) { 
                ...
                if (action.Effects.Any(requiredEffects.Contains))
                {
                    ...
                    // only use an action once (remove this if our version needs to repeat actions)
                    ...
                    // if all effacts at this depth have been satified, return true
